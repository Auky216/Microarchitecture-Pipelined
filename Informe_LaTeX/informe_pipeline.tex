...
No se modifico ni una sola linea de codigo en la \texttt{hunit.v}. Dado que el descompresor se ubico antes del registro \texttt{IF/ID}, la etapa Decode siempre recibe un bloque de 32 bits perfecto. La Hazard Unit procesa registros y emite senales de forwarding o stall exactamente de la misma manera que lo hacia antes.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../Images/waves_rvc_basic.png} % Placeholder
\caption{Formas de onda del test basico RVC. Se observa la mezcla de instrucciones \texttt{c.addi}, \texttt{add} normal y \texttt{c.sub}.}
\end{figure}

\textbf{Interpretacion de la forma de onda:}
\begin{itemize}
  \item \textbf{El salto de 2 en 2:} En la senal \texttt{PCF}, se puede observar claramente la secuencia de avance \texttt{0 -> 2 -> 4 -> 8}. El salto inicial de 2 en 2 bytes comprueba visualmente que el PC esta reconociendo la instruccion de 16 bits (\texttt{c.addi}) y avanzando adecuadamente.
  \item \textbf{Senal de Compresion:} La senal interna \texttt{is\_compressed} se eleva a alto (\texttt{1}) durante los ciclos donde el PC apunta a direcciones alineadas a media palabra.
\end{itemize}

\subsubsection{Test RVC de Memoria (\texttt{test\_rvc\_memory.mem})}
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../Images/waves_rvc_memory.png} % Placeholder
\caption{Formas de onda del test de memoria RVC. Se evalua el funcionamiento de las instrucciones \texttt{c.lw} y \texttt{c.sw}.}
\end{figure}

\textbf{Interpretacion de la forma de onda:}
\begin{itemize}
  \item A pesar de que las instrucciones en el codigo fuente ocupan solo 16 bits, la etapa Memory interactua con \texttt{Data Memory} transfiriendo 32 bits completos, tal como lo demuestra la validacion de las escrituras y lecturas exitosas en memoria. Las senales \texttt{MemWrite} se activan en los ciclos correctos.
\end{itemize}

\subsubsection{Test RVC de Saltos (\texttt{test\_rvc\_branches.mem})}
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../Images/waves_rvc_branches.png} % Placeholder
\caption{Formas de onda del test de saltos RVC. Ejecucion de branches condicionales \texttt{c.beqz} y \texttt{c.bnez}.}
\end{figure}

\textbf{Interpretacion de la forma de onda:}
\begin{itemize}
  \item Se visualiza el correcto calculo de offsets (saltos relativos al PC) para instrucciones comprimidas. A pesar de los 16 bits, el modulo Execute genera el Branch Target adecuado, y la Hazard Unit logra hacer el \texttt{Flush} de manera transparente ante un salto tomado.
\end{itemize}
